...
class BaseSingleTableSynthesizer:
    ...
    Args:
    metadata (dict):
        dictionary containing for each column their name and the type
...
class GaussianCopulaMethod(BaseSingleTableSynthesizer):
    ...
    def _fit(self, processed_data):
        ...
        """
        warn_missing_numerical_distributions(
            self.numerical_distributions, processed_data.columns
        )
    ...
    def _rebuild_gaussian_copula(self, model_parameters, default_params=None):
        ...
        for column, univariate in model_parameters["univariates"].items():
            ...
            if hasattr(model, "_argcheck"):
                ...
                LOGGER.debug(
                    f"Univariate for col '{column}' does not have _argcheck method."
                )
